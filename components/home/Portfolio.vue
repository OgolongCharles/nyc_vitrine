<template>
  <section class="commonSection porfolio">
    <div class="container">
      <div class="row">
        <div class="col-lg-12 text-center">
          <h4 class="sub_title">{{$t("Port1")}}</h4>
          <h2 class="sec_title">{{$t("Port2")}}</h2>
          <p class="sec_desc">

          </p>
        </div>
      </div>
      <div class="row scrole">
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/1.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/2.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/3.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/4.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/5.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/6.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/7.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/8.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/9.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/10.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/11.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/12.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/13.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/14.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/15.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/16.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/17.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/18.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/19.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/20.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/21.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/22.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/23.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/24.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/25.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
        <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/26.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
         <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/27.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div> <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/28.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div> <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/29.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div> <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Portfolio/30.jpg" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO</nuxt-link></h4>
            </div>
          </div>
        </div>
        <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Netflix.png" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Technologies,Business,Développement,Graphisme, CM, Formations</a>
              <h4><nuxt-link to="/portfolio-detail">Intégration Netflix</nuxt-link></h4>
            </div>
          </div>
        </div>
        <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/nkap_group.png" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Développement Web</a>
              <h4><nuxt-link to="/portfolio-detail">Vitrine NKAP GROUP</nuxt-link></h4>
            </div>
          </div>
        </div>
        <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/11.png" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Community Management </a>
              <h4><nuxt-link to="/portfolio-detail">CEF IMMO FINANZ</nuxt-link></h4>
            </div>
          </div>
        </div>
        <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/yamo.png" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Community Management </a>
              <h4><nuxt-link to="/portfolio-detail">YAMO AFRICA</nuxt-link></h4>
            </div>
          </div>
        </div>
        <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/12.png" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Développement Web & CM </a>
              <h4><nuxt-link to="/portfolio-detail">Batiservices</nuxt-link></h4>
            </div>
          </div>
        </div>
        <div class="col-lg-4 col-sm-6 col-md-4">
          <div class="singlefolio">
            <img src="/images/portfolio/Barn-Selam.png" alt=""/>
            <div class="folioHover">
              <a class="cate" href="#">Site de bonnes annonces</a>
              <h4><nuxt-link to="/portfolio-detail">SELAM</nuxt-link></h4>
            </div>
          </div>
        </div>

      </div>
    </div>
  </section>
</template>

<script>
    export default {
      name: "Portfolio",
            mounted() {
        if ($('.scrole').length > 0) {
          $('.scrole').owlCarousel({
            items: 4,
            loop:true,
            margin: 50,
            autoplay: true,
            nav: false,
            responsive: {
              0: {
                items: 1
              },
              560: {
                items: 2
              },
              800: {
                items: 2
              },
              992: {
                items: 3
              }
            }
          });
        }
      }

    }

</script>

<style scoped>
.commonSection{
  height:92vh !important;
}
.col-lg-4{
   width:350px;
}
.col-lg-4 img{
  height:320px;
  width:300px;
}
  @media(min-width:600px) and (max-width:800px){
    .commonSection{
height: 62vh !important;

}
  }
</style>
